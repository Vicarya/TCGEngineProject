...
| :--- | :--- | :--- | :--- |
| **T-01** | **ゲーム初期化処理 (`WeissGame.SetupGame`)**: プレイヤー作成、ゾーン初期化、デッキ読込とシャッフル、初期手札ドロー、マリガン処理を実装する。(マリガンは簡略版を実装。デッキ構築は仮実装のまま) | Gemini | `進行中` |
...
| **T-04** | **メインフェイズのアクションループ (`MainPhase.cs`)**: プレイヤーがアクション（カードプレイ、能力起動など）を任意に実行し、フェイズ終了を宣言できるループ処理を実装する。(カードプレイ時のレベル・コストチェック、カード種別判定を実装。能力起動は未実装) | Gemini | `進行中` |
| **T-05** | **アタックフェイズの基本処理 (`AttackPhase.cs`)**: アタック宣言、ダメージ計算、戦闘解決（リバース処理）の基本フローを実装する。（カウンター、アンコールは後回し。先行１ターン目のアタック制限を実装） | Gemini | `進行中` |
