...
| :--- | :--- | :--- | :--- |
| **T-01** | **ゲーム初期化処理 (`WeissGame.SetupGame`)**: プレイヤー作成、ゾーン初期化、デッキ読込とシャッフル、初期手札ドロー、マリガン処理を実装する。(デッキ構築は実装済み。マリガンは簡略版のまま) | Gemini | `進行中` |
| **T-01.1**| **マリガン処理の正式実装**: マリガンで控え室に置かれたカードを山札に戻してシャッフルする公式ルールを実装する。 | | `未着手` |
| **T-02** | **カードデータベースの構築**: カードデータを定義するJSON形式を決定し、ゲーム開始時に読み込む `CardLoader` クラスを作成する。(旧`CardDatabase`)。まずはサンプルカード10枚程度でよい。 | Gemini | `完了` |
| **T-03** | **基本フェーズ遷移の実装**: Stand -> Draw -> Clock -> Main -> Climax -> Attack -> End の自動遷移ロジックを `WeissRuleEngine` に実装する。 | Gemini | `完了` |
| **T-04** | **メインフェイズのアクションループ (`MainPhase.cs`)**: プレイヤーがアクション（カードプレイ、能力起動など）を任意に実行し、フェイズ終了を宣言できるループ処理を実装する。(カードプレイ時のレベル・コストチェック、カード種別判定、簡単な能力起動を実装) | Gemini | `完了` |
| **T-04.5**| **コアロジックの安定化**: コンパイルエラーを修正し、メインフェイズまでのロジックを安定させる。 | Gemini | `完了` |
| **T-05** | **アタックフェイズの基本処理 (`AttackPhase.cs`)**: アタック宣言、ダメージ計算、戦闘解決の基本フローを実装。 | Gemini | `完了` |
| **T-05.1** | **カウンターステップの実装**: 非ターンプレイヤーが助太刀カードを使用できるロジックを実装する。 | Gemini | `進行中` |
| **T-05.2** | **アンコールステップの実装**: バトルでリバースしたキャラをコストを支払って舞台に戻すアンコール処理を実装する。 | Gemini | `完了` |
| **T-05.3** | **アンコールコストの汎用化**: アビリティテキストからコスト（例: `[(1)]`）を解析し、処理するロジックを実装する。 | | `未着手` |
| **T-05.4** | **アンコールコスト支払い時のプレイヤー選択**: 手札のカードをコストにする際、どのカードを支払うかプレイヤーが選択できるようにする。 | | `未着手` |
